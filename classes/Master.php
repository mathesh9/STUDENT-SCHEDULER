...
class Master extends DBConnection
{
	...
	function save_payment()
	{
		...
		if ($check > 0) {
			$resp['status'] = 'failed';
			$resp['msg'] = 'The Account already have a payment details on the chosen month.';
			return json_encode($resp);
		}
		foreach ($_POST as $k => $v) {
	...
	function fetch_notifications()
	{
		...
		$sql = "SELECT title as message FROM schedule_list WHERE reminder_time <= '$current_time' AND user_id = $user_id AND reminder_time IS NOT NULL";
...
switch ($action) {
	...
	case 'fetch_notifications':
		echo $Master->fetch_notifications();
		break;
	default:
		// echo $sysset->index();
		break;
